...
function btn_click() {
  ...
  // id가 todo로 되어잇는 input tag에 사용자가 입력한 문자열을 추출하여
  // todo 변수에 담아라
  // input tag가 아닐경우에는 innerHTML 이라는 속성을 통해서 추출할수 있다.
...
// 즉 DOM 분서가 모두 화면에 그려진 후 JS 코드가 작동되도록 하닉위한
// 방법
// $(function(){  })
document.addEventListener("DOMContentLoaded", function () {
  ...
  // id가 btn_save인 tag(button)에 click event 핸들러 설정
  ...
  /*
    ...
    document.querySelectorAll() : class가 지정된 tag나 tag이름으로 선택할때 = 결과가 배열(여러개)
  ...
  */
  document.querySelector("#btn_save").addEventListener("click", function () {
    ...
    // 객체.value : input box일 경우에만 사용할수 있는 속성
    ...
    if (todo_value === "") {
      ...
      document.querySelector("section.todo_main form input[name='todo']");
      return false;
    }
    // 정상적으로 어떤 문자열이라도 입력을 수행했으면
    if (confirm("저장할까요?")) {
      // 화면에 form이 1개 뿐이기 때문에 Selector를 사용할수 있다
  ...
  */
  ...
  // id가 지정되지 않았을때
  // tag 이름으로 찾을경우는 같은이름의 tag가 여러개 있을 수 있기 때문에
  // 무조건 배열로 값이 추출된다.

$(function () {
  ...
  $("button.view-btn").click(function () {
    ...
    let id = $(this).data("id");
    // jquery 버전이  낮은 경우 data() 함수가 작동이 안되는 경우가 있다.
    // 그때는 아래와 같은 방법으로 id 값을 추출한다.
    // let id = $(this).attr("data-id")

    // text 문자열에 따라 가가 버튼의 역할 수행을 하도록
    if (text === "본문수정") {
      document.location.href = "/bbs/update/" + id;
    } else if (text === "삭제") {
      if (confirm("정말 삭제할까요 ><") === true) {
        // 삭제가 완료된 후 브라우저의 뒤로가기 버튼을 클릭했을 떄
        // 이미 삭제된 데이터가 보이는 것을 방지하기 위해서

...
이 내장클래스도 많은 이슈가 있어서 실제로 DB와 연동하여 사용할때
문제들을 일으킨다
그래서 내장 클래스가 있음에도 불구하고
...
router.get("/list", function (req, res) {
  // bbsVO Model 을 통해서 데이터를 모두 읽어오고(find(조건없음))
  ...
  bbsVO.find().then(function (bbsList) {
    ...
    // 전송을 한다.
    // 현재버전 14.X 버전에서는 { bbsList }라고 표현을 하면
...
router.get("/write", function (req, res) {
  ...
  // insert를 수행할때 비어있는 vo 를 만들어서 bbsWrite에 전달해주어야 한다.
  let data = new bbsVO();
...
router.post("/write", function (req, res) {
  // req.body객체에 input의 name 속성에 설정된 변수이름으로
  // 자동으로 담겨서 전송이 된다.
  ...
  // form에서 전송받은 데이터를 통째로 bbsVO(data) 객체로 생성하라
  // data에는 form에서 입력하여 전송한 데이터(작성자, 제목, 내용)와
  // 서버에서 임으로 추가한 데이터(날짜, 시간, 카운트)를 모두 모아서
  // 하나의 vo 객체로 생성을 하고
  // 거기에 DB DRUD와 관련된 함수들을 함께 추가하여 객체 생성을 한다.
  ...
  data
    ...
    .then(function (bbsVO) {
      // insert가 성공하면
      // res.json(bbsVO); //client에게 데이터를 보여라
      // 완료가 되면 리스트 화면으로 점프
    ...
    .catch(function (error) {
      ...
      // 웹 화면에 데이터 추가가 잘못되었다는
      // 메시지를 보여주는 코드를 수행해줘야 한다.
    });
...
// PK갑승ㄹ 가지고 tbl_bbs에서 1개의 item 값ㄷ을 추출하여
// detail view에 보여주기
router.get("/view/:id", function (req, res) {
  ...
  // findOne : findById()
  // { where : {_id:id} }
  // table의 _id값이 list 에서 전달받은 id 값과 일치하는 item이 있는지 검사
  bbsVO
...
조건문 없이 remove() 를 실행하면 테이블의 모든 데이터를 빠꾸없이 삭제해버린다
mongoDB 4.x 이상에서는 이러한 문제를 방지하기 위해
deleteOne() deletemany() 라는 함수를 만들어 두었다.
deleteOne(조건문)은 조건에 일치하는 데이터를 첫번째 한개만 삭제를 수행
...
router.get("/delete/:id", function (req, res) {
  ...
  bbsVO
    ...
    findOneDelete(조건문) : 삭제를 한 후 실제 삭제된 데이터를 
        다시 확인하고 싶을때
    */
    //.deleteOne({ _id: id })
...
/*
핸개의 item을 조회하여 write form으로 보낸 후 데이터를 다시 받았을때
req.body에는 id이 사라진 상태로 전달된다.
하지만 update를 수행할대 id 값을 URL에 전달하여 보냈기 때문에
for(method="POST",acrion="/bbs/update/id값") 형태의 URL이 자동으로 만들어지고
post method는 params에 id 값을 받아서 수신하는 결과가 된다.
...
router.post("/update/:id", function (req, res) {
  ...
  bbsVO
    .updateOne(
      { _id: id },
      {
        // input box에 입력한 데이터만 변경하도록 하는 코드
        // SET 에 해당하는 방법
        // 주의해야할 코드
        // $set { req.body }
